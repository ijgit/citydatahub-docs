...
### 1.1. Install Node.js and Postgresql11 | CentOS 7
...
2. 저장소 추가
   ...
   ```bash
   $ centos_version=7 # your centos version
   
   $ rpm -Uvh https://download.postgresql.org/pub/repos/yum/reporpms/EL-${centos_version}-x86_64/pgdg-redhat-repo-latest.noarch.rpm
   ```
...
5. 서비스 실행 및 상태 확인
   ...
   ```bash
   # 서비스 실행
   $ sudo systemctl start --now postgresql-11
...
### 1.2. Install Node.js and Postgresql11 | CentOS 8
...
1. 클라이언트 인증 설정 변경
   ...
   - METHOD 를 아래와 같이 수정하여 계정의 패스워드에 대한 서버로의 전송을 어떻게 할 것인지 설정합니다. 
      ...
      - password: text 로 password를 선송하는 것
