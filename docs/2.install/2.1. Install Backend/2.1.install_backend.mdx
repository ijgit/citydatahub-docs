...
<Tabs>
   ...
   [centos@localhost ~]$ sudo rpm -Uvh https://download.postgresql.org/pub/repos/yum/reporpms/EL-${centos_version}-x86_64/pgdg-redhat-repo-latest.noarch.rpm
   ```
...
backend 파일은 github 에서 다운받을 수 있습니다.
...
다운로드 받은 폴더 내의 `smartcity_back` 디렉토리 내의 파일들을 통해 backend를 설정하고 실행할 수 있습니다.
...
**5. 인증방식 변경**
   ...
   - postgresql 을 재시작
      ```bash
      [centos@localhost ~]$ sudo systemctl restart postgresql-11
      ```
